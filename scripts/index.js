...
async function getCertsFromEdition(editionId) {
  ...
  return await fetch(`https://api.verisart.com/v3/edition/${editionId}?size=250`, {
    "headers": headers,
    "method": "GET"
  }).then(async (res) => {
...
async function getAllCertsNotAssignedToken() {
  ...
  for(let i = 0; i < certs.length; i++) {
    try {
      ...
      if (tokenGateInfo === null) {
        ...
        } else if (type === 'Blossoming Cadaver' && !blossomingCoas.includes(token)) {
  ...
  fs.writeFile('schematic.json', JSON.stringify(schematicCoas), 'utf8', function(err){
    if(err) throw err;
  });
  fs.writeFile('blossoming.json', JSON.stringify(blossomingCoas), 'utf8', function(err){
    if(err) throw err;
  });
  fs.writeFile('mehretu.json', JSON.stringify(mehretuCoas), 'utf8', function(err){
    if(err) throw err;
  });
}
...
async function getAllCertsForAct() {
  return await fetch("https://api.verisart.com/v3/certificate?size=25&filter=ALL&sort=UPDATED_AT", {
    "headers": headers,
    "method": "GET"
  }).then(async (res) => {
    return await res.json()
...
async function getCertTransfer(certId) {
  return await fetch(`https://api.verisart.com/v3/certificate/${certId}/transfer`, {
    "headers": headers,
    "method": "GET"
  }).then(async (res) => {
    return await res.json()
